...
public class CommentService {
    ...
    public CommentDTO createComment(CreateCommentDTO request) {
        ...
        Comment comment = new Comment(request.getContent(), user, alien, parentComment);
        commentRepository.save(comment);

        return new CommentDTO(comment.getId(), comment.getContent(), user.getId(), user.getUsername(), alien.getId(), comment.getCreatedAt(), comment.getParent() != null ? comment.getParent().getId() : null);
    }
    ...
    public List<CommentDTO> getCommentsByAlien(Long alienId) {
        ...
        return commentRepository.findByAlienIdAndParentIsNull(alienId).stream()
                .map(comment -> new CommentDTO(comment.getId(), comment.getContent(),
                        comment.getUser().getId(), comment.getUser().getUsername(),
                        comment.getAlien().getId(), comment.getCreatedAt(),
    ...
    public CommentDTO updateComment(UpdateCommentDTO request, Long commentId) {
        ...
        return new CommentDTO(comment.getId(), comment.getContent(), comment.getUser().getId(), comment.getUser().getUsername(), comment.getAlien().getId(), comment.getCreatedAt(), comment.getParent() != null ? comment.getParent().getId() : null);
    }
